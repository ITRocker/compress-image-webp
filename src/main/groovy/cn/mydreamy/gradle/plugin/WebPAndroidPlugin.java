...
import cn.mydreamy.gradle.plugin.task.DecompressTask;
import cn.mydreamy.gradle.plugin.task.DownloadLibTask;
import cn.mydreamy.gradle.plugin.utils.Logger;
import cn.mydreamy.gradle.plugin.utils.SourceUtils;
...
public class WebPAndroidPlugin implements Plugin<Project> {
    ...
    private static final String EXTENSIONS_NAME = "png2webp";
    ...
    public void apply(Project project) {
        ...
        if (libFileName != null) {
            project.afterEvaluate(p -> {
                ...
                SourceUtils.addResourceDir(project, extension);
                ...
                for (String variantName : variantsName) {
                    Task convertTask = createConvertTask(p,
                            ...
                            extension.isUseOriPng(), extension.getQuality());
    ...
    private Task createConvertTask(Project project, String variant, String cwebpPath, boolean useOriPng, int quality) {
        ConvertTask task = project.getTasks().create("convert" + StringHelper.capitalize(variant) + "WebP",
                ConvertTask.class, new ConvertTask.ConfigAction(project, cwebpPath, useOriPng, quality));
        task.setGroup("webp");
        return task;
    }
...
//            }
            ...
            Logger.i("arch:" + arch);
            if ("32".equals(arch)) {
                return "libwebp-1.0.2-windows-x86-no-wic.zip";
            } else if ("64".equals(arch)) {
                return "libwebp-1.0.2-windows-x64-no-wic.zip";
            }
        }
        return null;
    }
...
//            }
            ...
            Logger.i("arch:" + arch);
            if ("32".equals(arch)) {
                path = "libwebp-1.0.2-windows-x86-no-wic/bin/cwebp.exe";
            } else if ("64".equals(arch)) {
                path = "libwebp-1.0.2-windows-x64-no-wic/bin/cwebp.exe";
            }
        }
